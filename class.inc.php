...
class analyse {
    ...
    function ajoute($nom) {
		   ...
		   $this->time = time();
//debug echo "<p>notion ajoutée $nom , il y en a ".$this->compte;
      }

    function relation($nom) {
           $this->relation[$this->compte] = $nom['relation'];
           $this->liennotion[$this->compte] = $nom['lien-notion'];
//debug echo "<p>relation ajoutée ".$nom['relation']." , il y en a ".$this->compte;
    }
...
//debug      echo "<p>chercheR ";
	  $cnt = $this->compte;
        for ($i=0; $i<$cnt; $i++) {
//debug      echo "<p>relation cherchée ".$this->relation[$i]." sur $i, il y en a ".$this->compte;
...
//				echo $this->item[$i]." ".$this->liennotion[$i];
      ...
      function paragraphe($type) {
        ...
        for ($i=0; $i<$cnt; $i++) {
                if ($type == "icone") {
                        if (isset($this->relation[$i])) {$tmp .= $this->relation[$i];}
                        if (isset($this->liennotion[$i])) {$tmp .= " ".$this->liennotion[$i];}
                        $tmp .= affichageIcone($this->item[$i], "actif");
                } else {
                       decodeDocument($this->item[$i]);
                       if ($partie == $partie0) { $partie1 = ""; }
                       else { $partie1 = $partie;}

                       if ($chapitre == $chapitre0) { $chapitre1 = ""; }
                       else { $chapitre1 = $chapitre;}

                       if ($souschapitre == $souschapitre0) { $souschapitre1 = ""; }
                       else { $souschapitre1 = $souschapitre;}
...
class pile {
	  ...
	  echo "<p>class.inc.php j'ajoute  $nom dans la pile";
           ...
           for ($i=0; $i<$this->compteur; $i++) {
               if ($this->item[$i] == $nom) { $flag = false; }
           }
           if ($flag) { $this->item[$this->compteur++] = $nom; }
      }
