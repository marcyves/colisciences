...
function list_content($eid) {
    ...
    $result = sql_query("select title, description from ".$prefix."_encyclopedia where eid='$eid' order by title, description", $dbi);
...
function content($tid, $ltr, $page=0, $query="") {
	...
	$result = sql_query("SELECT title from ".$prefix."_encyclopedia where eid='$ency[eid]' order by title", $dbi);
