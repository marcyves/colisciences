...
*				CODE CREER PAR  :
*
*- Ferran Romain                    de l'Ecole Centrale d'Electronique 
*- Vincent Le Druillennec           de l'Ecole Centrale d'Electronique
*
* Version 1.0
* date : 12/02/2004
* on s'est appuye sur le code préexistant du moteur de recherche 
****************************************************************************/

/*
*On verifie que les parametres sont bien passé
*
*/

//********************************Asupprimer
//$module_name="Bibliographie";
//*******************************************
...
*temporaire inutile
*
if(isset($_GET["disciplines"]))$disciplines=$_GET["disciplines"];
if(isset($_GET["domaines"]))$domaines=$_GET["domaines"];
*/
...
*temporaire inutile
*	
if(isset($_GET["quatriemeetou"]))$quatriemeetou=$_GET["quatriemeetou"];		
*/
...
/*
*switch principale de la page qui permet d'aller au moteur é un critére au mutlicritére
* par défaut mutlicritére
*/
...
//****************************debut de la fonction é un critére************************************
...
//global $module_name, $db;
	...
	OpenTable();
	include("BaseBiblio.html");
	CloseTable();
echo "<br>";
	OpenTable();
	print("<a href=\"parcours.php?name=$module_name&methrecherhe=mutli\">recherche é multicritére</a><br> \n");//&amp;file=search
...
function alpha($quoi) {
	...
	{
		$result = mysqli_query($dbi, "select $quoi from cb_biblio where $quoi LIKE '$ltr%'"); //------------->    UPPER($quoi)     );
	}
	elseif ($quoi == "Article" || $quoi == "Livre")
	{
		$result = mysqli_query($dbi, "select Titre from cb_biblio where Titre LIKE '$ltr%' AND Type LIKE '$quoi'");
	}
	else
	{
		$result = mysqli_query($dbi, "select Titre from cb_biblio where Titre LIKE '$ltr%'");
	}
	$count = sql_num_rows($result);

	//print("$count");
	if ( $count > 0) {
	    echo "<a href=\"?name=$module_name&amp;op=terms&amp;eid=$quoi&amp;ltr=$ltr&methrecherhe=simple\">$ltr</a><small>($count)</small>";// -----------> parcours.php
	} else {
	    echo "$ltr";
	}
...
//****************************fin de la fonction é un critére *************************************
...
//global $module_name, $db;
	...
	*/
	...
	print("<a href=\"parcours.php?methrecherhe=simple&name=$module_name\">recherche simple</a><br> \n"); 
...
/*
*Verification si il ya ua moin 1 des 3 critéres
*/
...
//*****************************************A SUPPRIMER********************
			print(" le nombre de resultat correspondant é la requete est : $num_rows .\n");
//********************************************************************
...
//				print("<td><a href=\"?name=$module_name&methrecherhe=content&tid=$row[Numero]\">$row[Titre]</a> : <br> $row[Auteurs], $row[Dates] <br>\n");
	...
	/*
	*Affichage d'un message indiquant via les critéres il n'y a aucune solution
	*/
			{
				echo "<tr><td> Il n'y a aucune donnée correspondant é votre demande (";
				if($titres !="")
				{
				...
				if($firstdates !="" && $seconddates !="")
				{
					...
					echo" et periode de = <i> $firstdates </i> é <i> $seconddates </i>";
				}
				else
				{
					if($firstdates !="")
...
global $module_name, $prefix, $sitename, $dbi, $admin;
//global $module_name;
print("<center><b> Rechercher é multiples critéres :</b></center>");

		print("<form name=\"form1\" method=\"post\" action=\"parcours.php?name=$module_name\">");
...
//*******************************************************************************
	...
	{		
		...
		print("<b> é &nbsp;</b><input class=inpt_main type=\"text\" name=\"seconddates\" size=\"4\" maxlength=\"4\" value=\"$seconddatesinfo\"></td></tr>");
