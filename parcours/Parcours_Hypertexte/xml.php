...
//
    /**
    * List of entities to be converted back.
    *
    * This array contains a list of entities to be converted when an XPath
    * expression is evaluated.
    *
    ...
    * @var    array
    */
    var $entitiesBack = array ( "&amp;" => "&", "&lt;" => "<", "&gt;" => ">",
        "&apos;" => "'", "&quot;" => '"' );
//Marc Augier end modification
    /**
...
// Marc Augier
// Converts back special characters
        $xml = strtr($xml, $this->entitiesBack);
// Marc Augier end of modification
        // Return the XML data.
        return $xml;
    }
	...
	{
	echo '<h2>$this->nodes[$context]["children"] pour '.$context.'</h2>';
	print_r($this->nodes[$context]["children"]);
	}    
    /**
    ...
    * @see       get_content(), evaluate()
    */
    function add_content2 ( $path, $value , $position)
    {
        // Check whether it's an attribute node.
        if ( ereg("/attribute::", $path) )
        {
        ...
        else
        {
            // Set the character data of the node.
            $this->nodes[$path]["text"] .= $value;
            $this->nodes[$path]["position"] .= $position."*";
        }
    }
...
// Marc Augier
// Converts back special characters
        $tmp = utf8_decode(strtr($this->nodes[$path]["text"], $this->entitiesBack));
// Marc Augier end of modification
...
//            return stripslashes($this->nodes[$path]["text"]);
    ...
    function get_position ( $path )
    {
		return stripslashes($this->nodes[$path]["position"]);
    }
...
//Marc Augier pour CoLiSciences
		$tmp = substr($path,26,4);
		if ($tmp=="note") {
			$tmp = substr($path,0,25);
            $this->nodes[$tmp]["text"] .= "<sup>(".$this->nodes[$path]["attributes"]['numappel'].")</sup> ";
		}
    }
    ...
    */
    function get_attributes ( $path )
    {
        ...
        return utf8_decode($this->nodes[$path]["attributes"]);
    }
    ...
    */
    ...
    {
		global $flag, $flagNotion, $texte, $texteNotion , $skip ;

//debug echo "<p>xml.php : start_element node $name";
        // Add a node.
        $this->path = $this->add_node($this->path, $name);
        // Set the attributes.
        $this->set_attributes($this->path, $attributes);

		if (($flag)&&($name!="notion")&&($name!="note")){
			$texte .= "<$name";
			if ($attributes!=""){
//debug print_r($attributes);
				if ($attributes[color]!=""){
					$texte .= " color=\"$attributes[color]\"";
				}
				if ($attributes[size]!=""){
					$texte .= " size=\"$attributes[size]\"";
				}
			}
			$texte .= ">";
		}
		...
		if ($name == "note"){
			...
			if ($attributes!=""){
//debug print_r($attributes);
				if ($attributes[numappel]!=""){
					$texte .= " <sup>$attributes[numappel]</sup>";
				}
			}
		}
		if ($name == "notion"){
//debug echo "<p>xml.php : start_element balise notion";
			$flagNotion = TRUE;
			$texteNotion .= "<$name";
			if ($attributes!=""){
//debug print_r($attributes);
				if ($attributes[relation]!=""){
					$texteNotion .= " relation=\"$attributes[relation]\"";
				}
				if ($attributes['lien-notion']!=""){
					$texteNotion .= ' lien-notion="'.$attributes['lien-notion'].'"';
				}
			} 
			$texteNotion .= ">";
		}
//debug echo "<p>xml.php : start_element retour";
    }
    ...
    */
    function handle_end_element ( $parser, $name )
    {
		...
		if (($flag)&&($name!="notion")&&!$skip){
			$texte .= "</$name>";
		}
		if ($flagNotion){
//debug echo "<p>xml.php : end_element balise notion";
			$texteNotion .= "</$name>";
		}
		if ($name == "notion"){
			$flagNotion = FALSE;
...
//debug  echo "<p>xml.php : end_element node $name";
        // Jump back to the parent element.
        $this->path = substr($this->path, 0, strrpos($this->path, "/"));
//debug  echo "<p>xml.php : end_element retour";
    }
    ...
    */
    ...
    {
		global $flag, $flagNotion, $texte, $texteNotion , $skip ;
		if (($flag)&&(!$flagNotion)&&!$skip){
			$texte .= $text;
		}
		if ($flagNotion){
			$texteNotion .= $text ;
		}
//debug  echo "<p>xml.php : character_data node $text";
        // Replace entities.
        $text = strtr($text, $this->entities);
        
        // Save the text.
        $this->add_content2($this->path, addslashes(trim($text)),xml_get_current_byte_index($parser));
//debug  echo "<p>xml.php : character_data retour";
    }
...
//        return call_user_method($method, $this, $node, $arguments);
    ...
    */
    function display_error ( $message )
    {
		...
		echo "<font class=\"title3\">Cet ouvrage n'est pas encore prét pour sa publication dans CoLiSciences.</font>";
//      echo "<h3>Désolé</h3>Le site est en construction et la page que vous voulez afficher n'est pas préte.<p><b>phpXML error:</b> ".$message;
