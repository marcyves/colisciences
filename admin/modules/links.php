...
$result = mysqli_query($dbi, "select radminlink, radminsuper from ".$prefix."_authors where aid='$aid'");
list($radminlink, $radminsuper) = mysqli_fetch_row($result);
...
function getparent($parentid,$title) {
    global $prefix,$dbi;
    $result=mysqli_query($dbi, "select cid, title, parentid from ".$prefix."_links_categories where cid=$parentid");
...
function links() {
    ...
    $result=mysqli_query($dbi, "select * from ".$prefix."_links_links");
...
/* Temporarily 'homeless' links functions (to be revised in admin.php breakup) */
    ...
    $result = mysqli_query($dbi, "select requestid,lid,cid,title,url,description,modifysubmitter from ".$prefix."_links_modrequest where brokenlink=1");
    $totalbrokenlinks = $result->num_rows;
    $result2 = mysqli_query($dbi, "select requestid,lid,cid,title,url,description,modifysubmitter from ".$prefix."_links_modrequest where brokenlink=0");
...
/* List Links waiting for validation */
    ...
    $result = mysqli_query($dbi, "select lid, cid, sid, title, url, description, name, email, submitter from ".$prefix."_links_newlink order by lid");
    $numrows = $result->num_rows;
    if ($numrows>0) {
	OpenTable();
	echo "<center><font class=\"option\"><b>"._LINKSWAITINGVAL."</b></font></center><br><br>";
	while(list($lid, $cid, $sid, $title, $url, $description, $name, $email, $submitter) = mysqli_fetch_row($result)) {
	    if ($submitter == "") {
		$submitter = _NONE;
	    }
	...
	$result2=mysqli_query($dbi, "select cid, title, parentid from ".$prefix."_links_categories order by title");
...
// Add a New Sub-Category
    ...
    $result = mysqli_query($dbi, "select * from ".$prefix."_links_categories");
    $numrows = $result->num_rows;
    if ($numrows>0) {
	OpenTable();
	echo "<form method=\"post\" action=\"admin.php\">"
	...
	$result2=mysqli_query($dbi, "select cid, title, parentid from ".$prefix."_links_categories order by parentid,title");
...
// Add a New Link to Database
    ...
    $result = mysqli_query($dbi, "select cid, title from ".$prefix."_links_categories");
    $numrows = $result->num_rows;
    if ($numrows>0) {
	OpenTable();
	echo "<form method=\"post\" action=\"admin.php\">"
...
/*
	...
	mysqli_query($dbi, "update ".$prefix."_links_links set cid=$cidto, sid=0 where cid='$cat[0]' AND sid='$cat[1]'");
*/
// end old categories

// begin new categories
// (comment lines to transfer existing datas)
    mysqli_query($dbi, "update ".$prefix."_links_links set cid=$cidto where cid=$cidfrom");
// end new categorie
...
function LinksModLink($lid) {
    ...
    GraphicAdmin();
    global $anonymous;
    $result = mysqli_query($dbi, "select cid, title, url, description, name, email, hits from ".$prefix."_links_links where lid=$lid");
    OpenTable();
    ...
    while(list($cid, $title, $url, $description, $name, $email, $hits) = mysqli_fetch_row($result)) {
    ...
    /* Modify or Add Editorial */
        ...
        $resulted2 = mysqli_query($dbi, "select adminid, editorialtimestamp, editorialtext, editorialtitle from ".$prefix."_links_editorials where linkid=$lid");
    ...
    OpenTable();
    /* Show Comments */
    $result5=mysqli_query($dbi, "SELECT ratingdbid, ratinguser, ratingcomments, ratingtimestamp FROM ".$prefix."_links_votedata WHERE ratinglid = $lid AND ratingcomments != '' ORDER BY ratingtimestamp DESC");
    ...
    $result5=mysqli_query($dbi, "SELECT ratingdbid, ratinguser, rating, ratinghostname, ratingtimestamp FROM ".$prefix."_links_votedata WHERE ratinglid = $lid AND ratinguser != 'outside' AND ratinguser != '$anonymous' ORDER BY ratingtimestamp DESC");
    ...
    while(list($ratingdbid, $ratinguser, $rating, $ratinghostname, $ratingtimestamp)=mysqli_fetch_row($result5)) {
    	...
    	$result2=mysqli_query($dbi, "SELECT rating FROM ".$prefix."_links_votedata WHERE ratinguser = '$ratinguser'");
    ...
    $result5=mysqli_query($dbi, "SELECT ratingdbid, rating, ratinghostname, ratingtimestamp FROM ".$prefix."_links_votedata WHERE ratinglid = $lid AND ratinguser = '$anonymous' ORDER BY ratingtimestamp DESC");
    $totalvotes = sql_num_rows($result5);
    ...
    $result5=mysqli_query($dbi, "SELECT ratingdbid, rating, ratinghostname, ratingtimestamp FROM ".$prefix."_links_votedata WHERE ratinglid = $lid AND ratinguser = 'outside' ORDER BY ratingtimestamp DESC");
    $totalvotes = sql_num_rows($result5);
...
function LinksDelComment($lid, $rid) {
    global $prefix, $dbi;
    mysqli_query($dbi, "UPDATE ".$prefix."_links_votedata SET ratingcomments='' WHERE ratingdbid = $rid");
    mysqli_query($dbi, "UPDATE ".$prefix."_links_links SET totalcomments = (totalcomments - 1) WHERE lid = $lid");
...
function LinksDelVote($lid, $rid) {
    global $prefix, $dbi;
    mysqli_query($dbi, "delete from ".$prefix."_links_votedata where ratingdbid=$rid");
    $voteresult = mysqli_query($dbi, "select rating, ratinguser, ratingcomments FROM ".$prefix."_links_votedata WHERE ratinglid = $lid");
    $totalvotesDB = sql_num_rows($voteresult);
    include ("voteinclude.php");
    mysqli_query($dbi, "UPDATE ".$prefix."_links_links SET linkratingsummary=$finalrating,totalvotes=$totalvotesDB,totalcomments=$truecomments WHERE lid = $lid");
    Header("Location: admin.php?op=LinksModLink&lid=$lid");
}
...
function LinksEditBrokenLinks($lid) {
    ...
    $result = mysqli_query($dbi, "select requestid, lid,cid,title,url,description, modifysubmitter from ".$prefix."_links_modrequest where brokenlink=1 order by requestid");
	list($requestid,$lid,$cid,$title,$url,$description,$modifysubmitter)= mysqli_fetch_row($result);
	$result4 = mysqli_query($dbi, "select name,email,hits from ".$prefix."_links_links where lid=$lid");
	list($name,$email,$hits)= mysqli_fetch_row($result4);	
    echo "<form action=\"admin.php\" method=\"post\">"
    ."<b>"._LINKID.": $lid</b><br><br>"
...
function LinksListBrokenLinks() {
    ...
    echo "<br>";
    OpenTable();
    $result = mysqli_query($dbi, "select requestid, lid, modifysubmitter from ".$prefix."_links_modrequest where brokenlink=1 order by requestid");
    $totalbrokenlinks = $result->num_rows;
	...
	echo "<center><font class=\"option\">"._NOREPORTEDBROKEN."</font></center><br><br><br>";
	    ...
	    $result2 = mysqli_query($dbi, "select title, url, submitter from ".$prefix."_links_links where lid=$lid");
...
function LinksDelBrokenLinks($lid) {
    global $prefix, $dbi;
    mysqli_query($dbi, "delete from ".$prefix."_links_modrequest where lid=$lid");
    mysqli_query($dbi, "delete from ".$prefix."_links_links where lid=$lid");
    Header("Location: admin.php?op=LinksListBrokenLinks");
}

function LinksIgnoreBrokenLinks($lid) {
    global $prefix, $dbi;
    mysqli_query($dbi, "delete from ".$prefix."_links_modrequest where lid=$lid and brokenlink=1");
    Header("Location: admin.php?op=LinksListBrokenLinks");
}
...
function LinksListModRequests() {
    ...
    echo "<br>";
    OpenTable();
    $result = mysqli_query($dbi, "select requestid, lid, cid, sid, title, url, description, modifysubmitter from ".$prefix."_links_modrequest where brokenlink=0 order by requestid");
    ...
    while(list($requestid, $lid, $cid, $sid, $title, $url, $description, $modifysubmitter)=mysqli_fetch_row($result)) {
	$result2 = mysqli_query($dbi, "select cid, sid, title, url, description, submitter from ".$prefix."_links_links where lid=$lid");
	list($origcid, $origsid, $origtitle, $origurl, $origdescription, $owner)=mysqli_fetch_row($result2);
	$result3 = mysqli_query($dbi, "select title from ".$prefix."_links_categories where cid=$cid");
	$result4 = mysqli_query($dbi, "select title from ".$prefix."_links_subcategories where cid=$cid and sid=$sid");
	$result5 = mysqli_query($dbi, "select title from ".$prefix."_links_categories where cid=$origcid");
	$result6 = mysqli_query($dbi, "select title from ".$prefix."_links_subcategories where cid=$origcid and sid=$origsid");
...
function LinksChangeModRequests($requestid) {
    global $prefix, $dbi;
    $result = mysqli_query($dbi, "select requestid, lid, cid, sid, title, url, description from ".$prefix."_links_modrequest where requestid=$requestid");
    while(list($requestid, $lid, $cid, $sid, $title, $url, $description)=mysqli_fetch_row($result)) {
	$title = stripslashes($title);
    	$description = stripslashes($description);
    	mysqli_query($dbi, "UPDATE ".$prefix."_links_links SET cid=$cid, sid=$sid, title='$title', url='$url', description='$description' WHERE lid = $lid");
    }
    mysqli_query($dbi, "delete from ".$prefix."_links_modrequest where requestid=$requestid");
...
function LinksCleanVotes() {
    global $prefix, $dbi;
    $totalvoteresult = mysqli_query($dbi, "select distinct ratinglid FROM ".$prefix."_links_votedata");
...
function LinksModLinkS($lid, $title, $url, $description, $name, $email, $hits, $cat) {
    ...
    mysqli_query($dbi, "update ".$prefix."_links_links set cid='$cat[0]', sid='$cat[1]', title='$title', url='$url', description='$description', name='$name', email='$email', hits='$hits' where lid=$lid");
...
function LinksModCat($cat) {
    ...
    if ($cat[1]==0) {
	$result=mysqli_query($dbi, "select title, cdescription from ".$prefix."_links_categories where cid=$cat[0]");
	...
	$result=mysqli_query($dbi, "select title from ".$prefix."_links_categories where cid=$cat[0]");
	list($ctitle) = mysqli_fetch_row($result);
	$result2=mysqli_query($dbi, "select title from ".$prefix."_links_subcategories where sid=$cat[1]");
...
function LinksModCatS($cid, $sid, $sub, $title, $cdescription) {
    global $prefix, $dbi;
    if ($sub==0) {
	mysqli_query($dbi, "update ".$prefix."_links_categories set title='$title', cdescription='$cdescription' where cid=$cid");
    } else {
	mysqli_query($dbi, "update ".$prefix."_links_subcategories set title='$title' where sid=$sid");
    }
    Header("Location: admin.php?op=Links");
}
...
function LinksDelCat($cid, $sid, $sub, $ok=0) {
    ...
    if($ok==1) {
	if ($sub>0) {
    	mysqli_query($dbi, "delete from ".$prefix."_links_categories where cid=$cid");
	    mysqli_query($dbi, "delete from ".$prefix."_links_links where cid=$cid");
	} else {
	    mysqli_query($dbi, "delete from ".$prefix."_links_links where cid=$cid");
		// suppression des liens de catégories filles
    	$result2 = mysqli_query($dbi, "select cid from ".$prefix."_links_categories where parentid=$cid");
    	while(list($cid2) = mysqli_fetch_row($result2)) {
			mysqli_query($dbi, "delete from ".$prefix."_links_links where cid=$cid2");
		}
	    mysqli_query($dbi, "delete from ".$prefix."_links_categories where parentid=$cid");
	    mysqli_query($dbi, "delete from ".$prefix."_links_categories where cid=$cid");
	}
	Header("Location: admin.php?op=Links");    
    } else {
	$result = mysqli_query($dbi, "select * from ".$prefix."_links_categories where parentid=$cid");
	$nbsubcat = $result->num_rows;

	$result2 = mysqli_query($dbi, "select cid from ".$prefix."_links_categories where parentid=$cid");
	while(list($cid2) = mysqli_fetch_row($result2)) {
		$result3 = mysqli_query($dbi, "select * from ".$prefix."_links_links where cid=$cid2");
...
function LinksDelNew($lid) {
    global $prefix, $dbi;
    mysqli_query($dbi, "delete from ".$prefix."_links_newlink where lid=$lid");
    Header("Location: admin.php?op=Links");
}
...
function LinksAddCat($title, $cdescription) {
    global $prefix, $dbi;
    $parentid=0;
    $result = mysqli_query($dbi, "select cid from ".$prefix."_links_categories where title='$title'");
    $numrows = $result->num_rows;
    if ($numrows>0) {
	include("header.php");
	GraphicAdmin();
	...
	mysqli_query($dbi, "insert into ".$prefix."_links_categories values (NULL, '$title', '$cdescription', $parentid)");
	Header("Location: admin.php?op=Links");
    }
}
...
function LinksAddSubCat($cid, $title, $cdescription) {
    global $prefix, $dbi;
    $result = mysqli_query($dbi, "select cid from ".$prefix."_links_categories where title='$title' AND cid='$cid'");
    $numrows = $result->num_rows;
    if ($numrows>0) {
	include("header.php");
	GraphicAdmin();
	...
	mysqli_query($dbi, "insert into ".$prefix."_links_categories values (NULL, '$title', '$cdescription', '$cid')");
	Header("Location: admin.php?op=Links");
    }
}
...
function LinksAddEditorial($linkid, $editorialtitle, $editorialtext) {
    ...
    mysqli_query($dbi, "insert into ".$prefix."_links_editorials values ($linkid, '$aid', now(), '$editorialtext', '$editorialtitle')");
    include("header.php");
    GraphicAdmin();
    OpenTable();
    echo "<center><br>"
...
function LinksModEditorial($linkid, $editorialtitle, $editorialtext) {
    global $prefix, $dbi;
    $editorialtext = stripslashes(FixQuotes($editorialtext));
    mysqli_query($dbi, "update ".$prefix."_links_editorials set editorialtext='$editorialtext', editorialtitle='$editorialtitle' where linkid=$linkid");
    include("header.php");
    GraphicAdmin();
    OpenTable();
    echo "<br><center>"
...
function LinksDelEditorial($linkid) {
    global $prefix, $dbi;
    mysqli_query($dbi, "delete from ".$prefix."_links_editorials where linkid=$linkid");
    include("header.php");
    GraphicAdmin();
    OpenTable();
    echo "<br><center>"
...
function LinksLinkCheck() {
    ...
    $result = mysqli_query($dbi, "select cid, title from ".$prefix."_links_categories order by title");
    ...
    $result = mysqli_query($dbi, "select sid, cid, title from ".$prefix."_links_subcategories order by title");
    while(list($sid, $cid, $title) = mysqli_fetch_row($result)) {
        $transfertitle = str_replace (" ", "_", $title);
    	$result2 = mysqli_query($dbi, "select title from ".$prefix."_links_categories where cid = $cid");
...
function LinksValidate($cid, $sid, $ttitle) {
	...
	$result = mysqli_query($dbi, "select lid, title, url, name, email, submitter from ".$prefix."_links_links order by title");
    }   	
	...
	$result = mysqli_query($dbi, "select lid, title, url, name, email, submitter from ".$prefix."_links_links where cid=$cid order by title");
    }
    /* Check Only Subcategory */
    if ($cid==0 && $sid!=0) {
   	echo "<tr><td colspan=\"3\"><center><b>"._VALIDATINGSUBCAT.": $transfertitle</b><br>"._BEPATIENT."</center><br><br></td></tr>";
   	$result = mysqli_query($dbi, "select lid, title, url, name, email, submitter from ".$prefix."_links_links where sid=$sid order by title");
    }
...
function LinksAddLink($new, $lid, $title, $url, $cat, $description, $name, $email, $submitter) {
    global $prefix, $dbi;
    $result = mysqli_query($dbi, "select url from ".$prefix."_links_links where url='$url'");
    $numrows = $result->num_rows;
    if ($numrows>0) {
	include("header.php");
	GraphicAdmin();
...
// Check if Description exist
    ...
    $name = stripslashes(FixQuotes($name));
    $email = stripslashes(FixQuotes($email));
    mysqli_query($dbi, "insert into ".$prefix."_links_links values (NULL, '$cat[0]', '$cat[1]', '$title', '$url', '$description', now(), '$name', '$email', '0','$submitter',0,0,0)");
