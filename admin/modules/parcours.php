...
function module_parcours() {
    ...
    while(list($mid, $title, $custom_title, $active, $view) = mysqli_fetch_row($result, $dbi)) {
        ...
        if ($title == $main_module) {
            ...
            $change_status = "<a href=\"admin.php?op=parcours_status&mid=$mid&active=$act\">$change</a>";
            $background = "";
        }
        echo "<tr><td $background>&nbsp;$title</td><td align=\"center\" $background>$custom_title</td><td align=\"center\" $background>$active</td><td align=\"center\" $background>$who_view</td><td align=\"center\" $background>[ <a href=\"admin.php?op=parcours_edit&mid=$mid\">"._EDIT."</a> | $change_status | $puthome ]</td></tr>";
    }
    echo "</table>";
    CloseTable();
    include ("footer.php");
...
function home_module($mid, $ok=0) {
    ...
    if ($ok == 0) {
        ...
        $result = sql_query("update ".$prefix."_modules_parcours set active='$active', view='$view' where mid='$mid'", $dbi);
        Header("Location: admin.php?op=parcours");
    }
}

function module_status($mid, $active) {
    global $prefix, $dbi;
    sql_query("update ".$prefix."_modules_parcours set active='$active' where mid='$mid'", $dbi);
    Header("Location: admin.php?op=parcours");
}
...
function module_edit($mid) {
    ...
    echo "<input type=\"hidden\" name=\"mid\" value=\"$mid\">"
        ."<input type=\"hidden\" name=\"op\" value=\"parcours_edit_save\">"
...
function module_edit_save($mid, $custom_title, $view) {
    global $prefix, $dbi;
    $result = sql_query("update ".$prefix."_modules_parcours set custom_title='$custom_title', view='$view' where mid='$mid'", $dbi);
    Header("Location: admin.php?op=parcours");
}
